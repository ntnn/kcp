...
func TestGarbageCollectorBlockingOwnerRefDoesBlock(t *testing.T) {
	...
	t.Logf("Waiting to confirm owner is NOT deleted (blocked by dependent)")
	// Give the GC time to process, then verify the owner still exists
	time.Sleep(10 * time.Second)

	// TODO find a better way to handle this. just waiting 10s is not good.
