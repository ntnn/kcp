...
func (c *Controller) reconcile(ctx context.Context, migration *migrationv1alpha1.LogicalClusterMigration) (bool, error) {
	...
	// Detect LC deletion during migration. Only meaningful while the origin
	// still owns the LC (Preparing/Migrating); after OriginCleanup the origin
	// has deleted its copy by design, so a missing LC there is the expected
	// steady state, not a deletion signal.
	if isOrigin && originOwnsLC(migration.Status.Phase) {
	...
	// Requeue on the origin shard during Migrating, where the destination is
	// active but the origin still owns the LC. This ensures the deletion
	// detection above re-runs even if the LC informer event is missed.
...
// the LogicalCluster's data, so a missing or deletion-marked LC is a genuine
// deletion signal. After OriginCleanup the origin deletes its copy by design,
// so those phases are excluded.
func originOwnsLC(phase migrationv1alpha1.LogicalClusterMigrationPhaseType) bool {
	switch phase {
	case migrationv1alpha1.LogicalClusterMigrationPhasePreparing,
		migrationv1alpha1.LogicalClusterMigrationPhaseMigrating:
		return true
	default:
		return false
	}
