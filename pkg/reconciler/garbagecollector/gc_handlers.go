...
func (gc *GarbageCollector) registerHandlers(ctx context.Context) func() {
	...
	// Question is whether that even matters as the requests only go to
	// the API server anyhow, which in turn goes to the backing etcd.
	// So even if the GC only handles "local" logical clusters it
	// doesn't matter much for traffic.
	// On top if the GC of all shards work on all logical clusters it
	// could help with balancing the gc load across shards.
...
func (gc *GarbageCollector) updateMonitors(oldCrd, newCrd *apiextensionsv1.CustomResourceDefinition) {
	...
	for gvr := range newVersions {
		...
		if _, exists := gc.monitors[gvr]; exists {
